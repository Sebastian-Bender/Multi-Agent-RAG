...
class AgentWorkflow:
    ...
    def build_workflow(self):
        ...
        workflow.add_node("final_node", self._final_node)
    ...
    def prepare_initial_state(self, question: str, retriever: EnsembleRetriever):
        try:
            print(f"[DEBUG] Preparing inital state with question='{question}'")
            documents = retriever.invoke(question)
            logger.info(f"Retrieved {len(documents)} relevant documents (from .invoke)")
    ...
    def _final_node(self, state: AgentState) -> AgentState:
        final_content = f"{state['draft_answer']}\n\n📋 **Verification Report:**\n{state['verification_report']}"
